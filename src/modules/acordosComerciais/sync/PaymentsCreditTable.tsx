...
import {
  Wallet, Loader2, AlertTriangle, Search, X, ArrowUpDown,
...
import type { PagamentoCredito, SortConfig } from './types';
...
const PAG_STATUSES: { value: string; label: string }[] = [
...
export function PaymentsCreditTable() {
  ...
  const filtered = useMemo(() => {
    ...
    if (q) {
      result = result.filter(
        (p) =>
          p.referenciaNf.toLowerCase().includes(q) ||
    ...
    if (filters.dateFrom) {
      ...
      result = result.filter((p) => !isBefore(parseISO(p.dataOperacao), from));
    }
    if (filters.dateTo) {
      const to = parseISO(filters.dateTo);
      result = result.filter((p) => !isAfter(parseISO(p.dataOperacao), to));
    }
    if (filters.valorMin) {
      const min = Number(filters.valorMin);
      if (!Number.isNaN(min)) result = result.filter((p) => p.valor >= min);
    }
    if (filters.valorMax) {
      const max = Number(filters.valorMax);
      if (!Number.isNaN(max)) result = result.filter((p) => p.valor <= max);
    }
    return result;
  }, [pagamentos, filters]);
  ...
  const sorted = useMemo(() => {
    ...
    return [...filtered].sort((a, b) => {
      const av = a[key as keyof PagamentoCredito];
      const bv = b[key as keyof PagamentoCredito];
    ...
    <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
      <div className="px-5 py-4 border-b border-gray-100">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-sm font-bold text-gray-800">Pagamentos e Créditos</h2>
              <p className="text-[11px] text-gray-500">
                {sorted.length} {sorted.length === 1 ? 'registro' : 'registros'} · Visão do Fornecedor
              </p>
            </div>
          </div>
          {hasFilters && (
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-2 mt-3">
          <div className="relative md:col-span-2">
            ...
            <input
              type="text"
              placeholder="Buscar por NF, empresa ou transação…"
      ...
      {loading ? (
        <div className="py-16 flex flex-col items-center justify-center gap-2">
          ...
          <p className="text-xs text-gray-500">Carregando pagamentos…</p>
        </div>
      ) : error ? (
        ...
        <div className="py-16 text-center">
          ...
          <p className="text-sm font-semibold text-gray-600">Nenhum pagamento encontrado</p>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50/60">
                  <SortableHeader label="Tipo" sortKey="tipo" sort={sort} onSort={cycleSort} />
                  <SortableHeader label="Empresa" sortKey="empresaNome" sort={sort} onSort={cycleSort} />
                  <SortableHeader label="Valor" sortKey="valor" sort={sort} onSort={cycleSort} align="right" />
                  <SortableHeader label="Data Operação" sortKey="dataOperacao" sort={sort} onSort={cycleSort} />
                  <SortableHeader label="NF Vinculada" sortKey="referenciaNf" sort={sort} onSort={cycleSort} />
              ...
              <tbody className="divide-y divide-gray-100">
                {paginated.map((pg) => (
                  <tr
                    key={pg.id}
                    onClick={() => setSelectedTransaction(pg.transactionId)}
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <p className="text-xs font-semibold text-gray-800 truncate max-w-[200px]">{pg.empresaNome || '—'}</p>
                    </td>
                    <td className="px-4 py-3 text-right">
                      <span className="text-xs font-bold text-gray-800 tabular-nums">{formatCurrency(pg.valor)}</span>
                    </td>
                    <td className="px-4 py-3">
                      <span className="text-xs text-gray-600 tabular-nums">{formatDate(pg.dataOperacao)}</span>
                    </td>
                    <td className="px-4 py-3">
                      <span className="text-xs font-mono text-gray-700">{pg.referenciaNf}</span>
                    </td>
