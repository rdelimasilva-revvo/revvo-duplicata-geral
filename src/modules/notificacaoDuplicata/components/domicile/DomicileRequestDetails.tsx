...
import { DomicileChangeRequest, Comment } from '@/modules/notificacaoDuplicata/types/domicile';
        ...
        <div className={`border rounded-lg p-4 mb-6 ${getStatusColor(request.status)}`}>
          <div className="flex items-center justify-between">
            ...
            <div className="text-sm">
              <span>Nível: {({ manager: 'Gerente', automatic: 'Automático', exception: 'Exceção', director: 'Diretor', analyst: 'Analista' }[request.approvalLevel as 'manager' | 'automatic' | 'exception' | 'director' | 'analyst']) ?? request.approvalLevel}</span>
              <span className="ml-4">Risco: {({ low: 'BAIXO', medium: 'MÉDIO', high: 'ALTO' }[request.riskScore as 'low' | 'medium' | 'high']) ?? request.riskScore.toUpperCase()}</span>
            </div>
          </div>
